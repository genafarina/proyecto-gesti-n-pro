...
-- General expenses are independent from projects.
...
-- Existing project records remain valid because both new columns are nullable.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_expenses_payment_account_id_fkey'
      AND conrelid = 'public.project_expenses'::regclass
  ) THEN
    ALTER TABLE public.project_expenses
      ADD CONSTRAINT project_expenses_payment_account_id_fkey
      FOREIGN KEY (payment_account_id)
      REFERENCES public.payment_accounts(id)
      ON DELETE SET NULL;
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_collections_payment_account_id_fkey'
      AND conrelid = 'public.project_collections'::regclass
  ) THEN
    ALTER TABLE public.project_collections
      ADD CONSTRAINT project_collections_payment_account_id_fkey
      FOREIGN KEY (payment_account_id)
      REFERENCES public.payment_accounts(id)
      ON DELETE SET NULL;
  END IF;
...
-- Keep updated_at consistent with the existing business tables.
...
CREATE TRIGGER trg_general_expenses_updated
  BEFORE UPDATE ON public.general_expenses
  FOR EACH ROW
  EXECUTE FUNCTION public.set_updated_at();

-- Do not duplicate seed accounts if the migration is replayed.
INSERT INTO public.payment_accounts (name, type)
SELECT seed.name, seed.type
FROM (
  VALUES
    ('Banco Macro', 'bank'),
    ('Mercado Pago', 'virtual_wallet'),
    ('Efectivo', 'cash'),
    ('Crédito', 'credit'),
    ('Otro', 'other')
) AS seed(name, type)
WHERE NOT EXISTS (
  SELECT 1
  FROM public.payment_accounts existing
...
CREATE INDEX IF NOT EXISTS general_expenses_expense_date_idx
  ON public.general_expenses(expense_date);
CREATE INDEX IF NOT EXISTS general_expenses_category_idx
  ON public.general_expenses(category);
CREATE INDEX IF NOT EXISTS general_expenses_expense_type_idx
  ON public.general_expenses(expense_type);
CREATE INDEX IF NOT EXISTS general_expenses_status_idx
  ON public.general_expenses(status);
CREATE INDEX IF NOT EXISTS general_expenses_payment_account_id_idx
  ON public.general_expenses(payment_account_id);
CREATE INDEX IF NOT EXISTS project_expenses_payment_account_id_idx
  ON public.project_expenses(payment_account_id);
CREATE INDEX IF NOT EXISTS project_collections_payment_account_id_idx
  ON public.project_collections(payment_account_id);
...
CREATE POLICY "payment_accounts select auth"
...
CREATE POLICY "payment_accounts insert admin"
  ON public.payment_accounts
  FOR INSERT
  TO authenticated
...
CREATE POLICY "payment_accounts update admin"
  ON public.payment_accounts
  FOR UPDATE
  TO authenticated
...
CREATE POLICY "payment_accounts delete admin"
  ON public.payment_accounts
  FOR DELETE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "general_expenses select auth"
...
CREATE POLICY "general_expenses insert admin"
  ON public.general_expenses
  FOR INSERT
  TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "general_expenses update admin"
  ON public.general_expenses
  FOR UPDATE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
...
CREATE POLICY "general_expenses delete admin"
  ON public.general_expenses
  FOR DELETE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));
